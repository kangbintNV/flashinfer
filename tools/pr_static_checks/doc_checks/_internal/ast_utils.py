...
def collect_module_alias_exports(
    ...
    only aliases that ultimately resolve to a ``@decorator``-decorated
    top-level function. Same shape as
    :func:`collect_flashinfer_api_functions` so the caller can union them.
    """
    decorated: dict[tuple[str, str], None] = {}
    for py_file in sorted(pkg_root.rglob("*.py")):
        try:
            tree = ast.parse(
                py_file.read_text("utf-8", "replace"), filename=str(py_file)
    ...
    for py_file in sorted(pkg_root.rglob("*.py")):
        ...
        for node in ast.walk(tree):
            ...
            for alias in node.names:
                if alias.name == "*":
                    continue
                if (target_mod, alias.name) in decorated:
